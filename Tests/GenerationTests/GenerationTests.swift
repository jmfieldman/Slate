...
class GenerationTests: XCTestCase {
  ...
  func testSingleClassFile() {
    let task = Process()
    task.launchPath = "/bin/sh"
    task.arguments = ["-c", "swift run slategen --input-model \(kModelPath) --output-slate-object-path \(kOutputPath)/Single --output-core-data-entity-path \(kCoreDataPath) -f --cast-int --name-transform Single%@ --file-transform SingleFile"]
    task.launch()
    task.waitUntilExit()
    XCTAssertEqual(task.terminationStatus, 0)
  }
